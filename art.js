...
function drawVisuals() {
    ...
    var colorA = [map(audioOsc || 0, 200, 0, 200, 255), map(audioOsc || 0, 100, 0, 140, 120), map(audioOsc || 0, 200, 0, 0, 30)];
    var colorB = [255, map(audioOsc || 0, 200, 0, 255, 175), map(audioOsc || 0, 200, 0, 255, 0)]; //oranje
    var colorC = [0, map(audioOsc || 0, 100, 0, 150, 220), map(audioOsc || 0, 100, 0, 150, 180)]; //cyan
    var colorD = [0, 0, 0, 40]; //zwart
...
function calculatePoints() {
    ...
    c0.points = Math.round(audioAvgOsc/5);
	if (c0.points < 2) { c0.points = c0.points + 2};
    c1.points = Math.round(getRandom(8, 24));
	if (c1.points < 2) { c1.points = c1.points + 1};
    c2.points = Math.round(getRandom(2, 12));
    if (c2.points < 2) { c2.points = c2.points + 1};
    c3.points = c2.points * (c2.points + 1);
    if (c3.points < 2) { c3.points = c3.points + 1};
    c4.points = c1.points;
    c5.points = c1.points;
    c6.points = c1.points;
...
function draw() {
    window.drawVisual(window.drawAudio());
}
